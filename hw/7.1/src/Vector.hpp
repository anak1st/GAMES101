...
public:
    ...
    static Vector3f Clamp(const Vector3f &v, const float &min, const float &max) {
        return Vector3f(std::clamp(v.x, min, max), std::clamp(v.y, min, max),
                        std::clamp(v.z, min, max));
    }

    static Vector3f Pow(const Vector3f &v, const float &exp) {
        return Vector3f(std::pow(v.x, exp), std::pow(v.y, exp), std::pow(v.z, exp));
    }
};
